...
m = hehnandrea(SimpleSchwarz.Optimizer,N,K;print_level=0,dt=dt,rho=1,omega=.1,tol=1e-6,
...
idx3 = []
for k in K
    push!(idx1,indexin(SimpleNL.index.(m[:x][:,1]),m[:Ws][k]))
    idx1[k]=idx1[k][idx1[k].!=nothing]
    push!(idx2,indexin(SimpleNL.index.(m[:x][:,3]),m[:Ws][k]))
    idx2[k]=idx2[k][idx2[k].!=nothing]
    push!(idx3,indexin(SimpleNL.index.(m[:x][:,5]),m[:Ws][k]))
