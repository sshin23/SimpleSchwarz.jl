...
using SimpleNL
import SimpleNL: variable, Model, Gradient, Constraint, fill_sparsity!, non_caching_eval, DUMMY, KKTErrorEvaluator
using SparseArrays
using LinearAlgebra
...
function hehnandrea(
    ...
    m = SimpleNL.Model(optimizer,K;opt...)
    ...
    objective(m,sum(sum(.5*Q[j]*(x[i,j]-d[i,j])^2 for j=1:n) + sum(.5*R[j]*(u[i,j]^2) for j=1:p) for i=1:N) + sum(.5*Qf[j]*(x[N+1,j]-d[N+1,j])^2 for j=1:n))
...
function thinplate(
    ...
    objective(m,
              sum(sum(sum(.5*Q[j,k]*(x[i,j,k]-d(i,j,k))^2 + .5*R[j,k]*(u[i,j,k]^2) for k=1:n) for j=1:n) for i=1:N) + sum(sum(.5*Qf[j,k]*(x[N+1,j,k]-d(N+1,j,k))^2 for k=1:n) for j=1:n))
...
function plot_err_profile(err_schwarz,omegas)
    k = 0
    markers = [:diamond,:circle,:square]
    err_schwarz = [get_errs_times(err)[1] for err in err_schwarz]
...
export hehnandrea, thinplate, plot_benchmark, plot_eds, savefig, plot_demonstration, plot_err_profile, KKTErrorEvaluator
